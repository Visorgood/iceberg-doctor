...
class CliSuite extends munit.FunSuite:
  ...
  test("refs takes a warehouse, a table and a limit") {
    assertEquals(
      Cli.parse(List("refs", "/tmp/wh", "prod.events.clicks")),
      Right(Invocation.Refs("/tmp/wh", "prod.events.clicks", Cli.DefaultLimit))
    )
  }
